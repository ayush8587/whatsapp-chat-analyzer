...
def fetch_stats(selected_user,df):
    ...
    positive_sentiment=(df.positive.sum()/(df.positive.sum() + df.negative.sum()))*100
    negative_sentiment=(df.negative.sum()/(df.positive.sum() + df.negative.sum()))*100

    return num_messages,len(words),num_media_messages,len(links),positive_sentiment,negative_sentiment
...
def create_wordcloud(selected_user,df):
    ...
    f = open('venv\stop_hinglish.txt', 'r')
    stop_words = f.read()
